...
from vilbert.task_utils_vqacp1 import (
    LoadDatasets,
    LoadLosses,
    ForwardModelsTrain,
    ForwardModelsVal,
...
def main():
    ...
    parser.add_argument(
        "--tasks", default="21", type=str, help="1-2-3... training task separate by -"
    )
    parser.add_argument(
        "--freeze",
        default=-1,
    ...
    if args.baseline:
        ...
        from vilbert.vilbert_for_vqacp1 import BertConfig
        from vilbert.vilbert_for_vqacp1 import VILBertForVLTasks
    ...
    for epochId in tqdm(range(start_epoch, args.num_train_epochs), desc="Epoch"):
        ...
        for step in range(median_num_iter):
            ...
            for task_id in task_ids:
                ...
                start.record()
                loss, batch_score = ForwardModelsTrain(
                        args,
                        task_cfg,
                        device,
                        task_id,
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    tbLogger.step_train(
                        ...
                        iterId,
                        float(loss),
                        float(total_scores / len(task_dataloader_train[task_id])),
...
def evaluate(
    ...
    for i, batch in enumerate(task_dataloader_val[task_id]):
        loss, batch_score, batch_size = ForwardModelsVal(
            args, task_cfg, device, task_id, batch, model, task_losses
        )
        tbLogger.step_val(
            epochId, float(loss), float(batch_score), task_id, batch_size, "test"
        )
        if default_gpu:
